...
from mineru_gateway.tasks.storage import cache_object_key, result_key
...
class CacheService:
    ...
    def compute_cache_key(self, file_records: list[tuple[str, str]], options: dict[str, Any]) -> tuple[str, str]:
        records_blob = json.dumps(file_records, separators=(",", ":"))
        options_blob = self._canonical_options(options)
        options_hash = hashlib.sha256(options_blob.encode()).hexdigest()
        key_material = f"{records_blob}\n{options_blob}".encode()
        return hashlib.sha256(key_material).hexdigest(), options_hash
    ...
    def _row_is_expired(self, row: CacheEntry) -> bool:
        return row.expires_at is not None and datetime.now(UTC) > ensure_aware_utc(row.expires_at)
    ...
    async def _reset_expired_row(
        ...
        *,
        ...
        if content_sha256 is not None:
            ...
            row.expires_at = datetime.now(UTC) + timedelta(seconds=ttl) if ttl else None
    ...
    async def _delete_stale_object(self, object_key: str | None) -> None:
        ...
        try:
            ...
            logger.exception("Failed to delete stale cache object %s", object_key)
    ...
    async def create_placeholder(
        ...
        *,
        ...
        async with get_db_session() as session:
            ...
            if row is not None:
                if self._row_is_expired(row):
                    stale_key = await self._reset_expired_row(
                        session,
                        row,
            ...
            row = CacheEntry(
                ...
                result_format=result_format,
                expires_at=(
                    datetime.now(UTC) + timedelta(seconds=ttl_seconds or self._settings.cache.ttl_seconds)
                    if (ttl_seconds or self._settings.cache.ttl_seconds)
                    else None
                ),
            )
            session.add(row)
    ...
    async def populate_from_task(self, task_id: str, *, cache_key: str, result_format: str) -> None:
        ...
        async with get_db_session() as session:
            ...
            if row is None:
                try:
                    await self._store.delete(dest)
                except Exception:
                    logger.exception("Failed to delete orphan cache object %s", dest)
                return
            ...
            try:
                ...
                try:
                    ...
                    logger.exception("Failed to delete orphan cache object %s after DB failure", dest)
                raise
    ...
    async def create_hit_task(self, *, task_id: str, cache: CacheEntry, file_names: list[str]) -> Task:
        ...
        try:
            ...
            if copied:
                try:
                    await self._store.delete(dest_key)
                except Exception:
                    logger.exception("Failed to delete orphan cache-hit result %s", dest_key)
            raise
    ...
    async def invalidate(self, cache_key: str) -> bool:
        ...
        if object_key:
            try:
                ...
                logger.exception("Failed to delete cache object %s during invalidation", object_key)
                return False
    ...
    async def sweep_expired(self) -> int:
        ...
        for row in rows:
            if row.object_key:
                try:
                    await self._store.delete(row.object_key)
                except Exception:
                    logger.exception("Failed to delete expired cache object %s", row.object_key)
                    continue
            async with get_db_session() as session:
