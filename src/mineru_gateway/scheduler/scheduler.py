...
class Scheduler:
    ...
    async def _reconcile_workers(self) -> None:
        ...
        discovered = await self._provider.discover(settings.deployment_id)
        ...
        for worker_id, instances in discovered_by_worker.items():
            ...
            discovered_primary[worker_id] = primary
            for inst in instances:
                if inst.instance_id == canonical_id:
                    continue
                await self._terminate_unowned_vm(
                    inst, reason=f"duplicate vm for worker {worker_id} (kept {canonical_id})"
                )

        for worker in workers:
            max_failures = self._settings.reconciliation.max_failure_count
            if worker.failure_count >= max_failures and worker.desired_state != "terminated":
                continue
            if (
    ...
    async def _reconcile_one_worker(self, worker: Worker, discovered) -> _ReconcileResult:
        ...
        if discovered is not None:
            ...
            if cloud_state != "unknown" and worker.cloud_state != cloud_state:
                await self._workers.commit_fields(worker.id, cloud_state=cloud_state)
                worker.cloud_state = cloud_state

        if worker.instance_id is None and worker.desired_state == "running":
            if discovered is not None:
                await self._workers.commit_fields(
                    worker.id,
                ...
                await self._workers.commit_fields(worker.id, instance_id=instance_id, cloud_state="pending")
                return _ReconcileResult(reset_cloud_retries=True, failure_recorded=False)

        if worker.instance_id is None:
            if worker.desired_state == "terminated":
                await self._workers.commit_fields(worker.id, cloud_state="terminated", terminated_at=now_utc())
            return _ReconcileResult(reset_cloud_retries=True, failure_recorded=False)

        state = await provider.get_state(worker.instance_id)
        cloud_state = cloud_state_from_instance(state)
        if cloud_state == "terminated" and worker.desired_state in ("running", "stopped"):
        ...
        if cloud_state != worker.cloud_state:
            await self._workers.commit_fields(worker.id, cloud_state=cloud_state)
            worker.cloud_state = cloud_state
            if cloud_state in ("stopped", "terminated"):
                await self._workers.commit_fields(
                    worker.id, base_url=None, ready_at=None, healthy=False, last_health_checked_at=now_utc()
                )

        if worker.desired_state == "running" and cloud_state == "stopped":
            await provider.start(worker.instance_id)
            await self._workers.commit_fields(
                worker.id,
                cloud_state="pending",
                failure_count=0,
        ...
        if worker.desired_state == "stopped" and cloud_state == "running":
            await provider.stop(worker.instance_id)
            await self._workers.commit_fields(worker.id, cloud_state="stopping")
            return _ReconcileResult(reset_cloud_retries=True, failure_recorded=False)

        if worker.desired_state == "terminated" and cloud_state not in ("terminated",):
            await provider.terminate(worker.instance_id)
            await self._workers.commit_fields(worker.id, cloud_state="terminating")
            return _ReconcileResult(reset_cloud_retries=True, failure_recorded=False)

        if worker.desired_state == "running" and cloud_state == "running":
            ip = await provider.get_private_ip(worker.instance_id)
    ...
    async def _converge_stalled_workers(self) -> None:
        ...
        for worker in await self._workers.find_stalled_workers():
            ...
            if not active_work:
                if worker.desired_state != "terminated":
                    await self._workers.commit_fields(worker.id, desired_state="terminated")
                    await self._workers.record_scaling_event_now(
                        action="terminate",
            ...
            if age_seconds >= grace_seconds and worker.desired_state != "terminated":
                await self._workers.commit_fields(worker.id, desired_state="terminated")
                await self._workers.record_scaling_event_now(
                    action="terminate",
    ...
    async def _poll_health(self, base_url: str) -> tuple[bool, str | None]:
        try:
            resp = await self._client.get(f"{base_url}/health", timeout=_HEALTH_TIMEOUT)
    ...
    async def _apply_autoscaling(self) -> None:
        ...
        if signal.should_scale_down:
            ...
            if idle is not None:
                await self._workers.commit_fields(idle.id, draining=True, drain_target="stopped")
                await self._workers.record_scaling_event_now(
    ...
    async def _scale_up(self) -> bool:
        ...
        if await self._workers.count_workers() < self._settings.scaling.max_workers:
            await self._workers.create_cloud_worker()
            return True
        ...
        for worker in await self._workers.find_draining_workers():
            ...
            if await self._workers.has_active_replacement_for(worker.id):
                if await self._workers.get_ready_replacement_for(worker.id) is None:
                    ...
                    continue
                await self._workers.commit_fields(worker.id, desired_state="terminated")
            elif worker.drain_target == "terminated":
                await self._workers.commit_fields(worker.id, desired_state="terminated")
            else:
                await self._workers.commit_fields(worker.id, desired_state="stopped", draining=False, drain_target=None)

    def _worker_is_ready(self, worker: Worker) -> bool:
        return (
            worker.desired_state == "running"
            and worker.cloud_state == "running"
            and worker.healthy
    ...
    async def _progress_rotations(self) -> None:
        ...
        for replacement in await self._workers.find_replacement_workers():
            ...
            if (
                replacement.desired_state != "terminated"
                ...
                if age > readiness_timeout:
                    await self._workers.commit_fields(replacement.id, desired_state="terminated")
                    logger.warning(
    ...
    async def _cleanup_if_due(self) -> None:
        ...
        if self._next_cache_sweep_at is None or now >= self._next_cache_sweep_at:
            ...
            await self._cache.sweep_expired()
