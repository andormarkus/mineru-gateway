...
async def _signal(db: AsyncSession) -> ScalingSignal:
    ...
    return compute_scaling_signal(
        queue_depth=await repo.count_queue_depth(),
        serviceable_workers=await repo.count_serviceable_workers(),
        starting_workers=await repo.count_starting_workers(),
...
async def test_max_workers_cap(db_session: AsyncSession) -> None:
    ...
    sig = compute_scaling_signal(
        queue_depth=20, serviceable_workers=1, starting_workers=0, target_per_worker=4, min_workers=0, max_workers=1
    )
    assert sig.desired_workers == 1
    assert not sig.should_scale_up
...
async def test_min_workers_floor_with_empty_queue(db_session: AsyncSession) -> None:
    sig = compute_scaling_signal(
        queue_depth=0, serviceable_workers=0, starting_workers=0, target_per_worker=4, min_workers=2, max_workers=12
    )
